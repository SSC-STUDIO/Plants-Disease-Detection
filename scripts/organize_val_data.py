"""
组织验证数据到正确的文件夹结构
根据标注文件将图像复制到 ./data/val/<class_id>/ 文件夹
"""
import os
import json
import shutil
...
# 路径配置
annotations_file = "data/temp/labels/val_split.json"
...
print("=" * 60)
print("Validation Data Organization Script")
print("=" * 60)
...
for class_id in class_counts.keys():
    class_dir = os.path.join(target_val_dir, str(class_id))
    os.makedirs(class_dir, exist_ok=True)
    print(f"  Created: {class_dir}")

# 复制文件
print(f"\nCopying images to {target_val_dir}/...")
copied = 0
missing = 0
errors = []
...
for ann in tqdm(annotations, desc="Copying"):
    ...
    # 目标文件路径
    dst_dir = os.path.join(target_val_dir, str(class_id))
...
# 验证结果
print(f"\nVerifying {target_val_dir} structure:")
for class_id in sorted(class_counts.keys()):
    class_dir = os.path.join(target_val_dir, str(class_id))
