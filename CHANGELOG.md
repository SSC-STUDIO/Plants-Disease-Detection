...
- **修复评估循环重复前向推理**：`evaluate_model` 每个 batch 先调用 `_predict_probabilities`（内部跑 `model(inputs)`），紧接着又调 `model(inputs)` 算 loss，导致同一张图做了两次前向。当 `tta_views=1` 时这是 50% 的无效计算。修复为先跑一次 `model(inputs)` 拿 logits 算 loss，若 TTA 关闭则直接从 logits softmax 得概率，避免二次推理。
